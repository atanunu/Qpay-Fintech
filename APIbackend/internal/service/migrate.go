...
func Migrate(ctx context.Context, db *sql.DB) error {
	...
	var prior string
	e = tx.QueryRowContext(ctx, `SELECT checksum FROM schema_migrations WHERE version=1`).Scan(&prior)
	if e == nil {
		if prior != checksum {
			return fmt.Errorf("schema version 1 checksum differs from applied migration")
		}
		return tx.Commit()
	}
	...
	if _, e = tx.ExecContext(ctx, `INSERT INTO schema_migrations(version,checksum) VALUES(1,$1)`, checksum); e != nil {
		return e
	}
	return tx.Commit()
}
