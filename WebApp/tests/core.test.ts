...
describe('payment recovery',()=>{
  ...
  it('finds an original quote through read-only history',async()=>{const request=vi.fn(async(method,path)=>{expect(method).toBe('GET');return [payment];});expect(await findOriginal({request,csrf:'',clear() {}} as Transport,intent)).toEqual(payment);expect(request).toHaveBeenCalledTimes(1);});
  it('returns not-found without creating a replacement payment',async()=>{const request=vi.fn(async()=>[]);expect(await findOriginal({request,csrf:'',clear(){}} as Transport,intent)).toBeNull();expect(request).toHaveBeenCalledTimes(1);});
