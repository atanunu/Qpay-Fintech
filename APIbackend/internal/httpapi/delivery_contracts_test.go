	...
	for path, methods := range paths {
		for method, value := range methods.(map[string]any) {
			...
			for _, value := range params {
				...
				if seen[key] {
					t.Fatalf("%s %s duplicates %s", method, path, key)
				}
				seen[key] = true
				if path == "/v1/statements" && (p["name"] == "from" || p["name"] == "to") {
					if p["required"] != true || p["schema"].(map[string]any)["format"] != "date-time" {
						t.Fatal("statement date constraints lost")
					}
				}
				if path == "/v1/payments" && p["name"] == "limit" && p["schema"].(map[string]any)["type"] != "integer" {
					t.Fatal("pagination integer schema lost")
				}
			}
		}
	}
