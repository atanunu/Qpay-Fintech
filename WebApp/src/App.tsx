import {CalendarPage,SchedulePage,SavedBillsPage,TokenArchivePage} from './pages/planner';
import {RequestsPage,RequestPage} from './pages/requests';
import {InsightsPage,SearchPage} from './pages/insights';
import {FundingCentrePage,DigitalIdentityPage} from './pages/funding_identity';
import {SecurityCentrePage,AccountPrivacyPage,ContactPage} from './pages/security_parity';
...
export default function App() {
  ...
  return <ScreenBoundary key={location.pathname}><Routes>
    ...
    <Route element={<SessionGate/>}><Route index element={<OverviewPage/>}/><Route path="/wallet" element={<WalletPage/>}/><Route path="/transfers" element={<TransferLanding/>}/><Route path="/transfers/new" element={<PaymentPage/>}/><Route path="/transfers/recovery" element={<RecoveryPage/>}/><Route path="/bills" element={<BillsPage/>}/><Route path="/bills/pay" element={<PaymentPage bill/>}/><Route path="/activity" element={<ActivityPage/>}/><Route path="/activity/:id" element={<PaymentDetailPage/>}/><Route path="/beneficiaries" element={<BeneficiariesPage/>}/><Route path="/statements" element={<StatementsPage/>}/><Route path="/notifications" element={<NotificationsPage/>}/><Route path="/notifications/view/:reference" element={<NotificationDestination/>}/><Route path="/verification" element={<DigitalIdentityPage/>}/><Route path="/verification/manual" element={<VerificationPage/>}/>
      <Route path="/settings" element={<SettingsLayout/>}><Route index element={<Navigate to="profile" replace/>}/><Route path="profile" element={<ProfilePage/>}/><Route path="security" element={<SecurityPage/>}/><Route path="devices" element={<DevicesPage/>}/><Route path="preferences" element={<PreferencesPage/>}/><Route path="privacy" element={<AccountPrivacyPage/>}/><Route path="contact" element={<ContactPage/>}/></Route>
      <Route path="/calendar" element={<CalendarPage/>}/><Route path="/calendar/schedule" element={<SchedulePage/>}/><Route path="/requests" element={<RequestsPage/>}/><Route path="/requests/:id" element={<RequestPage/>}/><Route path="/insights" element={<InsightsPage/>}/><Route path="/search" element={<SearchPage/>}/><Route path="/bills/saved" element={<SavedBillsPage/>}/><Route path="/bills/tokens" element={<TokenArchivePage/>}/><Route path="/funding" element={<FundingCentrePage/>}/><Route path="/security-centre" element={<SecurityCentrePage/>}/><Route path="/support" element={<SupportPage/>}/><Route path="/support/new" element={<NewCasePage/>}/><Route path="/support/:id" element={<CasePage/>}/><Route path="/review" element={<ReviewPage/>}/><Route path="*" element={<NotFound/>}/>
    </Route>
