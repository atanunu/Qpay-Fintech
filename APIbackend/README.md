...
**v0.4 working delivery:** runnable core implementation and automated tests, not a complete production release. Final corrected-branch verification and publication are still outstanding in this snapshot. Initial CI compiled/vetted the core; its first test run failed on synthetic staff email normalisation. The fixture is corrected in this workspace. Do not represent those corrections as a verified passing rerun until actual CI evidence exists.

## Purpose and boundaries
Serve customer and staff clients with persistent identity, wallet, payment, support and reporting operations. Unconfigured external execution returns unavailable; it never silently falls back to a simulator. The local profile uses unmistakably synthetic funds and bank/bill values. Actual funding rails, refunds/returns, full reconciliation, document storage and production acceptance remain open.
...
Implemented core source includes registration/challenges, sessions/refresh/revocation, MFA/recovery codes, transaction PINs, immutable quotes, idempotent internal transfers, external payment holds, query-only ambiguous recovery, scoped history, statements, receipts, bill fulfilment retrieval, support records, manual KYC/approval workflows and notification intents.

Remaining engineering is substantial and not merely a credentials task: partner account provisioning and verified funding callbacks; complete refund/return execution; full settlement reconciliation and treasury; private KYC document ingestion/ownership/scanning; production delivery events and opt-out; broader risk controls; contact-change/closure and staff invitation lifecycle; expanded pagination/exports; production database privilege separation, load/restore and independent security testing. Growth products are not implemented.

## Tests and evidence
[Validation status](docs/VALIDATION.md) distinguishes executed checks, the initial failed fixture, and pending rerun. First candidate evidence comes from GitHub's isolated PostgreSQL job, not an in-memory substitute. Client helper tests execute locally. New code is not labelled fully verified merely because it compiles. No real bank transfer or email delivery was performed.
...
## Changelog
v0.4: added persistent Go core, process entry points, client transport helper, local simulator, inspected QPay HTTP adapter, PostgreSQL/race tests, container setup and integration documentation. Final verification and publication status are recorded separately; no production activation is implied.
